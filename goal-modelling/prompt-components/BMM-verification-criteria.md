# BMM Verification Criteria

Use the following criteria to verify the internal quality of the submitted BMM goal model.
...
For each goal, examine whether:

* it describes a desired state of affairs;
* it expresses an end to be achieved rather than an activity to be performed;
* it is not formulated as a means, objective, influencer, requirement, or existing fact;
* its meaning is sufficiently clear and unambiguous;
* it is consistent with the other goals;
* it does not unnecessarily duplicate another goal;
* its level of abstraction is appropriate in relation to connected goals.

A formulation such as “Customers are satisfied with the service” describes a desired state. In contrast, “Conduct customer surveys” describes an activity and therefore has the character of a means rather than a goal.
