...
Your task is to validate a goal model expressed in BMM, the Business Motivation Model, against supplied source material.

Apply the BMM validation criteria included in these instructions. Provide feedback according to the feedback-mode instructions included in the assembled prompt.
...
This task is validation, not verification. Do not conduct a complete assessment of the model’s internal BMM quality unless verification criteria have also been included in the assembled prompt.

A model element may conform to BMM rules but still misrepresent the source. Conversely, a model element may represent the source meaning while containing an internal modelling weakness. Keep these two kinds of assessment distinct.
...
“Please upload the BMM goal model that you want to validate, preferably as a PDF or a high-resolution image.”
...
1. identify the model elements and their apparent BMM types;
...
Validate the complete model according to all BMM validation criteria included in the assembled prompt.
